...
private:
    vector<vector<POS_T>> make_turn(vector<vector<POS_T>> mtx, move_pos turn) const // функци€ выполнени€ хода
    {
        if (turn.xb != -1)
            mtx[turn.xb][turn.yb] = 0; // удаление шашки
        if ((mtx[turn.x][turn.y] == 1 && turn.x2 == 0) || (mtx[turn.x][turn.y] == 2 && turn.x2 == 7))
            mtx[turn.x][turn.y] += 2; // превращение шашки в ферз€
        mtx[turn.x2][turn.y2] = mtx[turn.x][turn.y]; // перемещение
        mtx[turn.x][turn.y] = 0; // "обнуление" координат
        return mtx;
    }

    double calc_score(const vector<vector<POS_T>> &mtx, const bool first_bot_color) const // оценка состо€ни€ доски
    {
...
public:
    void find_turns(const bool color) // публична€ перегрузка поиска хода на основе цвета
    {
        find_turns(color, board->get_board());
    }

    void find_turns(const POS_T x, const POS_T y) // публична€ перегрузка поиска хода на основе координат
    {
        find_turns(x, y, board->get_board());
    }

private:
    void find_turns(const bool color, const vector<vector<POS_T>> &mtx) // приватна€ перегрузка поиска хода на основе цвета и состо€ни€ доски 
    {
    ...
    void find_turns(const POS_T x, const POS_T y, const vector<vector<POS_T>> &mtx) // приватна€ перегрузка функции дл€ поиска хода по координатам клетки и состо€нию доски
    {
  ...
  public:
    ...
    int Max_depth; // максимальна€ глубина бота

  private:
    default_random_engine rand_eng; // движок рандомных значений дл€ генерации псевдо-случайных чисел
    string scoring_mode; // метод подсчета
    string optimization; // оптимизаци€ хода
    vector<move_pos> next_move; // следующий ход
    vector<int> next_best_state; // состо€ние следующего лучшего хода
    Board *board; // доска
